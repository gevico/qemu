...
static uint64_t k230_wdt_read(void *opaque, hwaddr addr, unsigned int size)
{
    ...
    case K230_WDT_COMP_PARAM_1:
        ...
        value = (32 << K230_WDT_CNT_WIDTH_SHIFT) |  /* 32-bit counter */
                (0 << K230_WDT_DFLT_TOP_INIT_SHIFT) | /* Default initial timeout */
                (0 << K230_WDT_DFLT_TOP_SHIFT) |    /* Default timeout */
                (K230_WDT_RPL_16_CYCLES << K230_WDT_DFLT_RPL_SHIFT) | /* Default RPL */
...
static void k230_wdt_realize(DeviceState *dev, Error **errp)
{
    ...
    ptimer_transaction_begin(s->timer);
    ptimer_set_freq(s->timer, 65536); /* 1Hz base frequency */
    ptimer_transaction_commit(s->timer);
}
